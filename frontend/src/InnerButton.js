...
function InnerButton(props) {
  ...
  const styles = {
    transform: `translate(${300 - width / 2}px) translateY(${300 - 20}px) rotate(${
      turnVar * props.index
    }turn) translateY(${-235}px)`,
    width: `${width}px`,
  };

  return <div className="inner-button" onClick={seatClick} style={styles}></div>;
}
